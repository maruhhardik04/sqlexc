...
DECLARE 
        ...
        COUNT(B.SYMPTOM1)+COUNT(B.SYMPTOM2)+COUNT(B.SYMPTOM3) as Total
        FROM TREATMENT A,DISEASE B 
        WHERE 
...
BEGIN
        ...
        IF :NEW.AVG_CURE_TIME < 1 THEN
            ...
            ELSIF updating then
                msg:=' update '; 
            END IF;
        RAISE_APPLICATION_ERROR(-20000,'You can not'||msg||'record if AVG_CURE_TIME is less than 1');
        END IF;               
END;
/
insert into TREATMENT values(7,5,'FLU',5,400,0);  
insert into TREATMENT values(7,5,'FLU',5,400,1);  