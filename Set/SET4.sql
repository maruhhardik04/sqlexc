...
--Project Data
INSERT INTO Project values(1,'project1','Running','21-mar-02',1); 
INSERT INTO Project values(2,'project2','Pending','20-jan-03',2); 
INSERT INTO Project values(3,'project3','Running','15-feb-04',3); 
INSERT INTO Project values(4,'project4','Running','16-sep-02',4); 
INSERT INTO Project values(5,'project5','Pending','17-oct-02',5); 
INSERT INTO Project values (6,'project6','Running','10-mar-03',6);
INSERT INTO Project values(7,'project7','Pending','05-sep-02',7);
INSERT INTO Project values(8,'project7','Pending','05-sep-02',7);
...
e.experience<=5 AND
e.dept_no=d.dept_no
order by d.dept_nocreate or replace function total_number_of_projects(status VARCHAR2)
RETURN NUMBER
IS
    c NUMBER;

BEGIN
    select COUNT(*) into c from Project where status=status;
    return c;
END;
...
create or replace function total_number_of_projects(tstatus VARCHAR2)
RETURN NUMBER
IS
    c NUMBER;

BEGIN
    select COUNT(*) into c from Project where status=tstatus;
    return c;
END;
/

-- Calling Function using Dual
select total_number_of_projects('Running') from dual;

-- Calling Function using PL/SQL Block
BEGIN
        dbms_output.put_line('Project : '||total_number_of_projects('Running'));
END;
/
...
is
    ...
    where start_date='21-MAR-02';
    --SELECT proj_id,type_of_project,status from project where start_date=(select to_char(sysdate,'dd-mon-yy') from dual);
BEGIN
        for r in c1 loop
...
BEGIN
        ...
        THEN
            ...
            ELSIF updating then
                ...
                msg:= ' delete '; 
            END IF;
        RAISE_APPLICATION_ERROR(-20000,'You can not'||msg||'record if status type is pending');
        END IF;               
END;
/
...
UPDATE project SET status='Pending' WHERE proj_id=7;